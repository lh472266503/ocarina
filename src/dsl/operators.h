...
#define OC_MAKE_DSL_BINARY_OPERATOR(op, tag, trait)                                                                           \
    ...
    decltype(std::declval<Lhs>() op std::declval<Rhs>()) trait##_func(Lhs lhs, Rhs rhs);                                      \
    template<typename Lhs, typename Rhs>                                                                                      \
    requires any_dsl_v<Lhs, Rhs>                                                                                              \
        Var<decltype(std::declval<expr_value_t<Lhs>>() op std::declval<expr_value_t<Rhs>>())> trait##_func(Lhs lhs, Rhs rhs); \
    template<typename Lhs, typename Rhs>                                                                                      \
    struct trait {                                                                                                            \
        using type = decltype(detail::trait##_func(std::declval<Lhs>(), std::declval<Rhs>()));                                \
    };                                                                                                                        \
    }                                                                                                                         \
    template<typename... T>                                                                                                   \
