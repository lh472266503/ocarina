...
[[nodiscard]] {device_flag} inline auto {prefix}faceforward({prefix}float3 n, {prefix}float3 i, {prefix}float3 n_ref) noexcept {{ return {prefix}dot(n_ref, i) < 0.0f ? n : -n; }}

[[nodiscard]] {device_flag} inline auto {prefix}transpose(const {prefix}float2x2 m) noexcept {{
    return {prefix}float2x2(m[0].x, m[1].x, m[0].y, m[1].y);
}}

[[nodiscard]] {device_flag} inline auto {prefix}transpose(const {prefix}float3x3 m) noexcept {{
    return {prefix}float3x3(m[0].x, m[1].x, m[2].x, m[0].y, m[1].y, m[2].y, m[0].z, m[1].z, m[2].z);
}}

[[nodiscard]] {device_flag} inline auto {prefix}transpose(const {prefix}float4x4 m) noexcept {{
    return {prefix}float4x4(m[0].x, m[1].x, m[2].x, m[3].x, m[0].y, m[1].y, m[2].y, m[3].y, m[0].z, m[1].z, m[2].z, m[3].z, m[0].w, m[1].w, m[2].w, m[3].w);
}}
